...
\begin{document}
  \begin{tikzpicture}[
    ...
    \node[foundation] (cc) at (0, 0)
      {\footnotesize\textsc{Book}\\[-.2em]\ncite{CC:Bernstein1987}};
    \node[foundation] (gray) at (40mm, 0)
      {\footnotesize\textsc{Book}\\[-.2em]\ncite{TP:Gray1993}};
    ...
    \node[font = \scriptsize, text = black!60] at (151mm, 25mm)
      {$\longrightarrow$\;cites};

    % Each arrow points from the citing work to the cited work.
    \draw[cite edge] (thesis.south west) -- (cc.north east);
    \draw[cite edge] (thesis.south east) -- (gray.north west);
    ...
    \draw[cite edge] (order.south west) to[out = 235, in = 40] (icde.east);
  \end{tikzpicture}
\end{document}
